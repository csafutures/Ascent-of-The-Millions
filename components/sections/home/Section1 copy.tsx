...
export default function Section1({ featured, trending }: { featured: any, trending: any }) {
  ...
  return (
    <>
      {/* only show this section if there is featured or trending content */}
      {(featured || trending) && (
                    ...
                    <div className="col-sm-12 col-md-6">
                      <article className="first mb-3">
                                ...
                                <div
                                    dangerouslySetInnerHTML={{
                                      __html: DOMPurify.sanitize(
                                        truncate(featured.mainArticle.content, 150, { ellipsis: "..." })
                                      ),
                                    }}
                                  />
                              ) : null}
                        ...
                        <div className="entry-meta align-items-center">
                          <Link href={`/author/${featured?.mainArticle?.author?.id}`}>{featured?.mainArticle?.author?.user.first_name} {featured?.mainArticle?.author?.user.last_name}</Link> in <Link href={`/categories/${featured?.mainArticle?.category?.slug}`}>{featured?.mainArticle?.category?.name}</Link>
                          <br />
                          <span>
